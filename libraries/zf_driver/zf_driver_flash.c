...
uint8 flash_check(flash_section_enum sector_num, flash_page_enum page_num) {
    uint16 temp_loop = 0;
    uint32 flash_addr =
        ((FLASH_BASE_ADDR + FLASH_SECTION_SIZE * sector_num + FLASH_PAGE_SIZE * page_num));  // 提取当前 Flash 地址
...
uint8 flash_erase_page(flash_section_enum sector_num, flash_page_enum page_num) {
    ...
    do { time_out--; } while ((0x00000001 == FLASH->SR) && (0x00 != time_out));  // 等待超时或者完成
...
void flash_read_page_to_buffer(flash_section_enum sector_num, flash_page_enum page_num) {
    ...
    {
        flash_union_buffer[temp_loop].uint32_type =
            *(__IO uint32 *)(flash_addr + temp_loop * 4);  // 循环读取 Flash 的值
    }
}
...
//-------------------------------------------------------------------------------------------------------------------
void flash_buffer_clear(void) { memset(flash_union_buffer, 0xFF, FLASH_PAGE_SIZE); }
